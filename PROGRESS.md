...
**Phase 7a — the durable layer — is BUILT and verified locally (2026-09-20), and NOT pushed.**
The owner's patient-first billing brief (which supersedes the earlier POS prompt) needed four
additive migrations and the write paths behind the four sale types: **D-074** (a patient is a
`customers` row with a server-generated code, and one `admissions` table carries IPD credit) and
**D-075** (a pharmacy sale's rate is tax-inclusive, GST extracted from it, and a balance is charges
less allocated money). Files: `20260920000033_phase7a_hospitals_and_doctors.sql`,
...
assertions, 0 FAIL**). **The Flutter flow is NOT started** — no POS screen, controller or widget in
the app has changed, so nothing about the app's behaviour is different yet, and that is the next
slice. **Nothing was pushed to the hosted project** (the brief forbids it in this task), so the four
migrations are written, locally executed and untested against the hosted database.
...
| N-15 | **What a package sale multiplies, and what it charges tax on — both still unstated.** The rate is `cost × (1 + pharmacies.package_markup_percent/100)`, and "cost" is resolved as the batch's **landed cost** where one is recorded, falling back to its **purchase rate** — a fallback the opening-stock catalogue (Phase 6.5a) requires, since it has no landed cost. D-070 records the two candidates as different numbers, so the fallback is a **choice made to keep the flow working, not a settled basis**. Its **tax treatment** is likewise unsettled: a package line needs its product's slab recorded and is **refused** without one rather than inheriting the counter's 5% default — a refusal, not an answer. **The four per-pharmacy markup values are also still the owner's to supply** (D-070); until then a package sale is refused, naming the setting | Medium | Ask the owner: (a) landed cost or invoice purchase rate, (b) is a package sale a taxed B2B supply, and if so on what slab, (c) the four markup percentages. Each answer is a small migration/seed, not a redesign |
| N-16 | **`supabase/tests/phase5_alerts.sql` has one assertion that has been failing since migration 00031 — it is not a regression, and it is not fixed here.** Assertion 3 asserts *"expiry_date is NOT NULL in the schema, so no batch can reach the alert without one"*, and 00031 (Phase 6.5a, 2026-09-20) deliberately dropped that NOT NULL so an imported batch with no expiry could be represented honestly. Measured 2026-09-20 both ways on a local Postgres: with all 36 migrations **24 PASS / 1 FAIL**, and with migrations 00001-00032 only (no Phase 7a at all) **24 PASS / 1 FAIL** — identical, so Phase 7a neither caused nor changed it. **This means the Phase 5 alert test file is currently red**, which that chunk's own gate list (SQL tests are run by hand against the hosted project) never re-ran | Low | Rewrite the assertion to what is true now (a batch may have no expiry; `expiry_status` reports `'unknown'` and the dashboard's three buckets exclude such rows — `20260920000031:118-140`), or delete it and assert the new behaviour instead. One file's worth of work, in the chunk that owns the alert sources |
| N-17 | **Two follow-ups Phase 7a deliberately did not do.** (a) **A patient master edit is not permission-controlled**: the brief asks that updating an existing master stay permission-controlled, but `customers` carries the standard tenant-wide four-policy template, so a cashier can edit one today. Narrowing it changes an existing policy for an existing screen and was left out of this slice (D-074). (b) **`sales.discount_above_limit_request_id` does not exist**, because its target `approval_requests` does not (D-071), so an above-10% discount is **refused** rather than recorded — the protected branch is **not production-ready** and a cashier who needs one is blocked until Phase 6.5c lands. Both are recorded rather than silently spanned | Medium | (a) a policy change plus the customer-edit screens' role gate, in the slice that owns the RBAC question; (b) Phase 6.5c, which D-071/D-067 already make a hard prerequisite |
